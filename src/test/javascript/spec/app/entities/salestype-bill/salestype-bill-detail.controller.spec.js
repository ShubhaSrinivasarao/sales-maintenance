'use strict';

describe('Controller Tests', function() {

    describe('SalestypeBill Management Detail Controller', function() {
        var $scope, $rootScope;
        var MockEntity, MockPreviousState, MockSalestypeBill, MockSalestypeAmount, MockSalestypeBillDetl;
        var createController;

        beforeEach(inject(function($injector) {
            $rootScope = $injector.get('$rootScope');
            $scope = $rootScope.$new();
            MockEntity = jasmine.createSpy('MockEntity');
            MockPreviousState = jasmine.createSpy('MockPreviousState');
            MockSalestypeBill = jasmine.createSpy('MockSalestypeBill');
            MockSalestypeAmount = jasmine.createSpy('MockSalestypeAmount');
            MockSalestypeBillDetl = jasmine.createSpy('MockSalestypeBillDetl');
            

            var locals = {
                '$scope': $scope,
                '$rootScope': $rootScope,
                'entity': MockEntity,
                'previousState': MockPreviousState,
                'SalestypeBill': MockSalestypeBill,
                'SalestypeAmount': MockSalestypeAmount,
                'SalestypeBillDetl': MockSalestypeBillDetl
            };
            createController = function() {
                $injector.get('$controller')("SalestypeBillDetailController", locals);
            };
        }));


        describe('Root Scope Listening', function() {
            it('Unregisters root scope listener upon scope destruction', function() {
                var eventType = 'salesMaintenanceApp:salestypeBillUpdate';

                createController();
                expect($rootScope.$$listenerCount[eventType]).toEqual(1);

                $scope.$destroy();
                expect($rootScope.$$listenerCount[eventType]).toBeUndefined();
            });
        });
    });

});
